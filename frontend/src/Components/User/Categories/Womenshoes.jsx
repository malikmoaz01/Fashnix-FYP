...
const WomenShoes = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for Women's Shoes products only
        const womenShoesProducts = data.filter(product => 
          product.subcategory === "Women's Shoes"
        );
        
        setProducts(womenShoesProducts);
        setFilteredProducts(womenShoesProducts);
        setLoading(false);
      } catch (err) {
  ...
  const priceBrackets = [
    ...
    { label: 'Rs1,000 - Rs3,000', value: [1000, 3000] },
    { label: 'Rs3,000 - Rs5,000', value: [3000, 5000] },
    { label: 'Over Rs5,000', value: [5000, 100000] }
  ];
  ...
  }, [searchQuery, shoeType, priceRange, sortOption, products]);
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 md:p-6">
      <h1 className="text-3xl font-bold text-center mb-6 text-pink-600">Women's Shoes Collection</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-12">
              ...
              <p className="text-center text-gray-500">No women's shoes products match your search criteria</p>
              <button 
                onClick={resetFilters}
