...
const MenShoes = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for Men's Shoes products only
        const menShoesProducts = data.filter(product => 
          product.subcategory === "Men's Shoes"
        );
        
        setProducts(menShoesProducts);
        setFilteredProducts(menShoesProducts);
        setLoading(false);
      } catch (err) {
  ...
  // Extract shoe types from schema subsubcategory enum for men's shoes
  const shoeTypes = ["Dress Shoes", "Casual Shoes", "Slipers", "Boots", "Sneakers", "Sandals"];
